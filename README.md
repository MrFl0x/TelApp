...
**Telegram Mini App** для поиска сожителей: анкета с фото, условной логикой
и модерацией прямо в Telegram. Без бэкенда — только статичный HTML/CSS/JS
и Supabase.

![HTML/CSS/JS](https://img.shields.io/badge/frontend-vanilla%20JS-f7df1e?logo=javascript&logoColor=black)
![Supabase](https://img.shields.io/badge/backend-Supabase-3ecf8e?logo=supabase&logoColor=white)
![Telegram Mini App](https://img.shields.io/badge/platform-Telegram%20Mini%20App-26A5E4?logo=telegram&logoColor=white)
![No backend server](https://img.shields.io/badge/backend%20server-none-lightgrey)
...
- [Модерация анкет в Telegram](#модерация-анкет-в-telegram)
- [Структура проекта](#структура-проекта)
- [Схема данных](#схема-данных)
- [Доступ и безопасность](#доступ-и-безопасность--кто-может-достучаться-до-базы)
- [Быстрый старт](#быстрый-старт)
- [Настройка уведомлений модератору](#настройка-уведомлений-модератору)
- [Исходное ТЗ](#исходное-тз)

## Что это

Пользователь открывает Mini App внутри Telegram, заполняет анкету (тип
жилья, образ жизни, фото, контакты) и отправляет её. Анкета и фото
сохраняются напрямую в Supabase из браузера — отдельного сервера нет.
Новая анкета уходит модератору в Telegram, который одной кнопкой
принимает или отклоняет её; принятые анкеты публикуются в каналы.

## Возможности

- 📋 Форма анкеты с условной логикой (общежитие / съёмная квартира) без
  перезагрузки страницы
- 🖼️ Загрузка 1–5 фото с превью, удалением и валидацией размера/типа
- 🎨 Автоподстройка под тему Telegram (`Telegram.WebApp.themeParams`) —
  светлая и тёмная тема
- ☑️ Отправка через нативный `MainButton`, а не HTML-кнопку
- 🗄️ Хранение анкет и фото в Supabase (Postgres + Storage), без
  собственного backend-сервера
- 🔐 Разграничение доступа через Row Level Security (RLS) в Postgres
- 🤖 Уведомление модератора в Telegram с фото и кнопками «✅ Принять /
  ❌ Отклонить»
- 📢 Автопубликация принятых анкет в Telegram-каналы
...
```
TelApp/
├── index.html                          # форма (UI, валидация, условная логика, отправка в Supabase)
└── supabase/
    ├── schema.sql                      # таблица, RLS-политики, Storage-бакет для фото
    ├── migrations/                     # миграции для уже существующей базы
    └── functions/
        ├── _shared/                    # общее форматирование анкеты в текст
        ├── notify-moderator/           # шлёт новую анкету модератору в Telegram
        └── telegram-webhook/           # обрабатывает нажатия кнопок ✅/❌, /start
```
...
Уведомления модератору в Telegram — отдельная, необязательная настройка,
см. следующий раздел.
...
<details>
<summary>Показать оригинальное техническое задание</summary>
...
4. Фотографии:
    - Загрузка от 1 до 5 фото (input type="file", accept="image/*", multiple)
    - Превью загруженных фото сразу на странице (миниатюры)
    - Возможность удалить выбранное фото до отправки
    - Валидация: минимум 1 фото, максимум 5 — не давать загрузить больше и не давать
